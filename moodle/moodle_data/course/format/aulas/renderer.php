...
/**
 * format_aulas_renderer
 *
 * @package    format_aulas
 * @author     Rodrigo Brandão (rodrigobrandao.com.br)
 * @copyright  2017 Rodrigo Brandão
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_aulas_renderer extends format_topics_renderer{
    ...
    protected function get_button_section($course, $sectionvisible)
    {
        ...
        if ($colorcurrent = $this->get_color_config($course, 'colorcurrent')) {
            $css .=
            '#aulas-container-section .aulas-button-section.current {
        ...
        if ($colorvisible = $this->get_color_config($course, 'colorvisible')) {
            $css .=
            '#aulas-container-section .buttonsection.sectionvisible {
        ...
        $inline = '';
        $container_home .= html_writer::start_tag('div',['class'=>'container-fluid','id' => 'aulas-modules-container']);
        $container_home .= html_writer::start_tag('div',['class'=>'row featurette']);

        /** Modules title */
        $container_home .= html_writer::start_tag('div');

        $container_home .= html_writer::tag('h5','Módulos',['class' => "aulas-title-modules",]);
        $container_home .= html_writer::tag('p','En esta sección podrás encontrar los módulos ligados a tu guía',['class' => "aulas-subtitle-module",]);

        $container_home .= html_writer::end_tag('div');

        /* cards */
        $cardscontainerhtml .= html_writer::start_tag('div', ['class' => "col-md-12 aulas-container-cards",'style' => "margin:1rem 0;display:flex"]);
        
        // $percent = progress_percentage_aulas($course->id);
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            $id = 'aulas-button-section-' . $section;
            ...
            if($module == 1){
                ...
                $modulecardhtml .= html_writer::tag('h5','Módulo profesor',['class' => "aulas-title-dashboard", 'style'=>'display:'.$classvisibleifnotteacher.'']);

                $modulecardhtml .= html_writer::start_tag('div', ['class' => "col-md-12 p-0 d-flex"]);


                $modulecardhtml .= html_writer::start_tag('div', ['class' => "card aulas-card-module   col-md-4 ",'style'=>'display:'.$classvisibleifnotteacher.';margin-left: 0.2rem;margin-right: 0.5rem;', 'id'=>'aulas-card-module-'.$module]);
                
                // $act = $percent[$section]->section;
                $user = $USER->id;
                // $the_Activities = activities_aulas($course->id,$act,$user);
                //$sec++;
                $class = 'aulas-button-section';
                
                
                //div con las imagenes de los modulos
                $onclick = 'M.format_aulas.show(' . $section . ',' . $course->id . ')';   
                ...
                $modulecardhtml .= html_writer::start_tag('div', ['class' => "col-md-12 p-0 aulas-module-images-dashboard" , 'id' => "dashboard-aulas-".$module.""]);
                $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                // $aulas_images_module_1 = 'format/aulas/img/M1.png';
                // $img_aulas_1 =  '<img src="' . $aulas_images_module_1  . '" id="module-image-1" />';
                // $modulecardhtml .=  $img_aulas_1;
                ...
                //fin del div con las imagenes de los modulos
                     ...
                     //$module_tittle = get_string('Modulo','format_aulas');
                    // $btn_ir = get_string('ir','format_aulas');
                    /* inicio del texto de las cards del modulo */
                    $modulecardhtml .= html_writer::start_tag('div', ['id' => $id, 'class' => "card-body aulas-card-module-body"]);
                    $cant_char_name = strlen($name);
                    if($cant_char_name > 30){
                        $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                        $modulecardhtml .= html_writer::tag('h5',substr($name, 0, 30).'...',['class' => "card-title_aulas"]);
                        $modulecardhtml .= html_writer::end_tag('a');
                    }
                    else{
                        $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                        $modulecardhtml .= html_writer::tag('h5',$name,['class' => "card-title_aulas"]);
                        $modulecardhtml .= html_writer::end_tag('a');
                    }
                    
                    $cant_char =strlen($desc);
                    if($cant_char > 100){
                        $modulecardhtml .= html_writer::tag('p',substr($desc, 0, 100).'...',['class' => "card-text_aulas"]);
                    }
                    else{
                        $modulecardhtml .= html_writer::tag('p',$desc,['class' => "card-text_aulas"]);
                    }

                    $modulecardhtml .= html_writer::start_tag('div', ['onclick' => $onclick, 'class' => 'aulas-btn-start-modules']);

                    $modulecardhtml .= html_writer::start_tag('a',['onclick' => $onclick]);
                    $modulecardhtml.= get_string('button_modules', 'format_aulas');;
                    $modulecardhtml .= html_writer::tag('i', '', ['class' => 'm-2 fa fa-arrow-circle-right']);
                    $modulecardhtml .= html_writer::end_tag('a');
                    ...
                    //     $modulecardhtml .= html_writer::end_tag('div');
                    //     $modulecardhtml .= html_writer::end_tag('div');
                    //     $modulecardhtml .= html_writer::tag('p',$the_Activities.'% '.get_string('completado','format_aulas'),['class'=>"progress-bar-p"]);
                    //     }
                    $modulecardhtml .= html_writer::end_tag('div');
                ...
                if($module == 2){
                    ...
                    $modulecardhtml .= html_writer::tag('h5','Módulos estudiantes',['class' => "aulas-title-dashboard"]);
                    $modulecardhtml .= html_writer::end_tag('div');
                }
                
                $modulecardhtml .= html_writer::start_tag('div', ['class' => "card aulas-card-module   col-md-3 ",'style'=>'display:flex;margin-left: 0.2rem;margin-right: 1.5rem;', 'id'=>'aulas-card-module-'.$module]);
                ...
                $modulecardhtml .= html_writer::start_tag('div', ['class' => "col-md-12 p-0 aulas-module-images-dashboard", 'id' => "dashboard-aulas-".$module.""]);
                $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                // $aulas_images_modules = 'format/aulas/img/M'.$module.'.png';
                // $img_aulas_modules = '<img src="' . $aulas_images_modules  . '" id="module-image-'.$module.'" />';
                // $modulecardhtml .= $img_aulas_modules;
                $modulecardhtml .= html_writer::end_tag('a');
                $modulecardhtml .= html_writer::end_tag('div');
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            $course_btn_back_lang = get_string('back', 'format_aulas');
            $onbackclick = 'M.format_aulas.back(' . $course->id . ')';
            $backnavhtml .= html_writer::start_tag('nav', ['class' => "navbar sticky-top navbar-light aulas-back-navbar", 'id' => 'aulas-back-nav-'.$modules]);
            $backnavhtml .= html_writer::start_tag('a',  ['class' => 'btn btn-primary btn-sm aulas-back-nav', 'onclick' => $onbackclick]);
            $backnavhtml .= html_writer::tag('i', '', ['class' => 'fa fa-arrow-left']);
            $backnavhtml .= $course_btn_back_lang;
            $backnavhtml .= html_writer::end_tag('a');
            $aulas_images_activities = 'format/aulas/img/BANNER-ACTIVIDAD.png';
            $img_aulas = '<img src="' . $aulas_images_activities . '" id="module-image" />';
            $backnavhtml .= $img_aulas;
            /** lessons title */
            $backnavhtml .= html_writer::start_tag('div');

            $backnavhtml .= html_writer::tag('h5','Actividades',['class' => "mt-4 aulas-title-modules",]);
            $backnavhtml .= html_writer::tag('p','En esta sección podrás encontrar las actividades  ligados a tu módulo',['class' => "aulas-subtitle-module",]);
         ...
         */
           $html .= $backnavhtml;
           $html = html_writer::tag('div', $html, ['id' => 'aulas-container-section']);
           if ($PAGE->user_is_editing()) {
               $load = 'M.format_aulas.showmodule()';
               $html .= html_writer::tag('div', get_string('editing', 'format_aulas'), ['class' => 'alert alert-warning alert-block fade in']);
    ...
    {   
        global $PAGE;
        if ($PAGE->user_is_editing()) {
            $class_editing ='aulas-editing';
            return html_writer::start_tag('ul', ['class' => 'buttons row '.$class_editing. '','id'=>'rowinit']);
        }
        return html_writer::start_tag('ul', ['class' => 'buttons row aulas-no-editing','id'=>'rowinit']);
    }
    /**
     * section_header
     *
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused)
    {
        ...
        //     $nameobj = (string)$course->{'moduleobjetive' . ($currentdivisor2-1)};
        // }
        // $course_Desempeno_lang = get_string('desempeno', 'format_aulas');
       ...
       // echo "<p id='charge'>".get_string('cargando', 'format_aulas')."</p>";
        ...
        if (!$PAGE->user_is_editing()) {
            $PAGE->requires->js_init_call('M.format_aulas.init', [$course->numsections,$course->id]);
            echo html_writer::tag('style', '#summary { display: block !important; }');
