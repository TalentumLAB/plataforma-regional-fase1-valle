 ...
 */
 M.format_bilinguismo = M.format_bilinguismo || {
    ourYUI: null,
    numsections: 0,
    nuActualSection: 0
};
M.format_bilinguismo.locationurl = function(idcourse){
...
M.format_bilinguismo.Organizar = function(){
...
M.format_bilinguismo.init = function (Y, numsections,idcourse) {
    this.ourYUI = Y;
    ...
    var section = document.getElementById('bilinguismo-container-section');
    section.setAttribute('class', 'col-sm-12');
    section.style.display = 'block';
    // document.getElementById('section-0').style.display = 'block';
    document.getElementById('bilinguismo-modules-container').style.display = 'block';
    ...
    for(let i=1; i <= this.numsections; i++){
        ...
        if(moduleElement){
            moduleElement.addEventListener('mouseenter', e => {
                document.getElementById('bilinguismo-button-section-'+i).classList.add('after-class-'+i);
    
            });
            moduleElement.addEventListener('mouseleave', e => {
                document.getElementById('bilinguismo-button-section-'+i).classList.remove('after-class-'+i);
...
M.format_bilinguismo.hide = function () {
    for (var i = 1; i <= this.numsections; i++) {
        if (document.getElementById('bilinguismo-button-section-' + i) != undefined) {
            var buttonsection = document.getElementById('bilinguismo-button-section-' + i);
...
M.format_bilinguismo.fullhide = function () {
    this.hide();
    if (document.getElementById('bilinguismo-button-section-' + 0) != undefined) {
        var buttonsection = document.getElementById('bilinguismo-button-section-' + 0);
        this.hidehome();
    }
};
M.format_bilinguismo.show = function (id, courseid) {
    this.fullhide();
    this.nuActualSection = id;
    var buttonsection = document.getElementById('bilinguismo-button-section-' + id);
    var currentsection = document.getElementById('section-' + id); 
    // var nonvideo = document.querySelector('#container-'+id+">#nonvideo");
    // var sectioninternal = document.getElementById('bilinguismo-container-section');
    document.getElementById('above').style.display='none !important';
    ...
    for (let i = 0; i < textCard.length; i++) {
        ...
        if(innerTextName.length > 45){
            innerTextName = innerTextName.substring(0,45);
    ...
    actmodulesA.forEach((function(x){
        x.classList.add('formatbilinguismoiconlink');
    }));
    actmodulesIMG.forEach((function(x){
        x.classList.add('formatbilinguismoiconimage');
    }));
    actmodulesSPAN.forEach((function(x){
        x.classList.add('formatbilinguismoiconspan');
    }));
    this.hidehome();
    this.showback(id);
    window.scrollTo({ top: 0, behavior: 'smooth' });
};
M.format_bilinguismo.showmodule= function(){
    document.getElementById('charge').style.display='none';
    document.getElementById('above').style.display='none !important';
...
M.format_bilinguismo.hidehome = function () {
    document.getElementById('section-' + 0).style.display = 'none';
    document.getElementById('bilinguismo-modules-container').style.display = 'none';
}
M.format_bilinguismo.hideback = function () {
    for (var i = 1; i <= this.numsections; i++) {
        document.getElementById('bilinguismo-back-nav-'+i).style.display = 'none';
        document.getElementById('container-'+ i).style.display= 'none';    
    }
}
M.format_bilinguismo.showback = function (id) {
    if(this.nuActualSection != id){
    document.getElementById('bilinguismo-back-nav-'+id).style.display = 'none';
    }else{
    document.getElementById('bilinguismo-back-nav-'+id).style.display = 'block';
    // document.getElementById('bilinguismo-back-nav-'+id).style.marginBottom = '3rem';
    document.getElementById('bilinguismo-back-nav-'+id).style.padding = '.5rem 0rem';
    }
}
M.format_bilinguismo.back = function (courseid) {
    this.hide();
    document.getElementById('section-' + 0).style.display = 'block';
    document.getElementById('bilinguismo-modules-container').style.display = 'block';
    var sectioninternal = document.getElementById('bilinguismo-container-section');
...
M.format_bilinguismo.h5p = function () {
...
M.format_bilinguismo.Coursecontent = function($url){
    window.open(""+$url+"");
};
M.format_bilinguismo.Courseprocess = function($url){
    window.open(""+$url+"");
};
M.format_bilinguismo.Forum = function($url){
    window.open(""+$url+"");
};
M.format_bilinguismo.Meeting = function($url){
    window.open(""+$url+"");
};
M.format_bilinguismo.Doubts = function($url){
    window.open(""+$url+"");
};