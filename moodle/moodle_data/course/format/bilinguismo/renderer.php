...
/**
 * format_bilinguismo_renderer
 *
 * @package    format_bilinguismo
 * @author     Rodrigo Brandão (rodrigobrandao.com.br)
 * @copyright  2017 Rodrigo Brandão
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_bilinguismo_renderer extends format_topics_renderer{
    ...
    protected function get_button_section($course, $sectionvisible)
    {
        ...
        if ($colorcurrent = $this->get_color_config($course, 'colorcurrent')) {
            $css .=
            '#bilinguismo-container-section .bilinguismo-button-section.current {
        ...
        if ($colorvisible = $this->get_color_config($course, 'colorvisible')) {
            $css .=
            '#bilinguismo-container-section .buttonsection.sectionvisible {
        ...
        $inline = '';
        $container_home .= html_writer::start_tag('div',['class'=>'container-fluid','id' => 'bilinguismo-modules-container']);
        $container_home .= html_writer::start_tag('div',['class'=>'row featurette']);
        /* cards */
        $cardscontainerhtml .= html_writer::start_tag('div', ['class' => "col-md-12 bilinguismo-container-cards",'style' => "margin:1rem 0;display:flex"]);
        
        $percent = progress_percentage_bilinguismo($course->id);
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            $id = 'bilinguismo-button-section-' . $section;
             ...
             */
                ...
                if($module == 1){
                    ...
                    $modulecardhtml .= html_writer::start_tag('div', ['class' => "col-md-12 d-flex mt-4 justify-content-center"]);
                    $modulecardhtml .= html_writer::tag('h5','Modules',['class' => "bilinguismo-title-dashboard"]);
                    $modulecardhtml .= html_writer::end_tag('div');
                }
                
                $modulecardhtml .= html_writer::start_tag('div', ['class' => "card bilinguismo-card-module   col-md-3 ",'style'=>'display:flex;margin-left: 0.2rem;margin-right: 1rem;', 'id'=>'bilinguismo-card-module-'.$module]);
            
            
                $act = $percent[$section]->section;
                $user = $USER->id;
                $the_Activities = activities_bilinguismo($course->id,$act,$user);
                //$sec++;
                $class = 'bilinguismo-button-section';
                $onclick = 'M.format_bilinguismo.show(' . $section . ',' . $course->id . ')';   
                ...
                try {
                    $courseimage = '';
                    $fs = get_file_storage();
                    $files1 = $fs->get_area_files($context->id, 'format_bilinguismo', 'images', $img);
                    ...
                    if ($files1) {  
                        ...
                        $isimage = $file1->is_valid_image();
                            if ($isimage) {
                                ...
                                $modulecardhtml .=html_writer::empty_tag('img', array('src' => $courseimage,'alt'=>'','id'=>'module-image-'.$module.''));
                            }
                            else{
                                //no hacer nada
                            }
                ...
                // $bilinguismo_images_modules = 'format/bilinguismo/img/M'.$module.'.png';
                // $img_bilinguismo_modules = '<img src="' . $bilinguismo_images_modules  . '" id="module-image-'.$module.'" />';
                // $modulecardhtml .= $img_bilinguismo_modules;
                $modulecardhtml .= html_writer::end_tag('a');
                $modulecardhtml .= html_writer::end_tag('div');

                //$module_tittle = get_string('Modulo','format_bilinguismo');
                // $btn_ir = get_string('ir','format_bilinguismo');
                /* inicio del texto de las cards del modulo */
                $modulecardhtml .= html_writer::start_tag('div', ['id' => $id, 'class' => "card-body bilinguismo-card-module-body"]);
                $cant_char_name = strlen($name);
                if($cant_char_name > 45){
                    $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                    $modulecardhtml .= html_writer::tag('h5',substr($name, 0, 45).'...',['class' => "card-title"]);
                    $modulecardhtml .= html_writer::end_tag('a');
                }
                else{
                    $modulecardhtml .= html_writer::start_tag('a', ['onclick' => $onclick]);
                ...
                //     $modulecardhtml .= html_writer::end_tag('div');
                //     $modulecardhtml .= html_writer::end_tag('div');
                //     $modulecardhtml .= html_writer::tag('p',$the_Activities.'% '.get_string('completado','format_bilinguismo'),['class'=>"progress-bar-p"]);
                //     }
                $modulecardhtml .= html_writer::end_tag('div');
                $modulecardhtml .= html_writer::end_tag('div');
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            if($module_pri == 1){
                $bilinguismo_images_kid = 'format/bilinguismo/img/kid_text_module_complete.png';
                $img_bilinguismo_kid = '<img src="' . $bilinguismo_images_kid . '" style="width:100%;max-width:800px" />';
                $cardscontainerhtml .= $img_bilinguismo_kid;
            }
            $module_pri ++;
        } 
        $cardscontainerhtml .= html_writer::end_tag('div');
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            $course_btn_back_lang = get_string('back', 'format_bilinguismo');
            $onbackclick = 'M.format_bilinguismo.back(' . $course->id . ')';
            $backnavhtml .= html_writer::start_tag('nav', ['class' => "navbar sticky-top navbar-light bilinguismo-back-navbar", 'id' => 'bilinguismo-back-nav-'.$modules]);
            $backnavhtml .= html_writer::start_tag('a',  ['class' => 'bilinguismo-back-nav', 'onclick' => $onbackclick]);
            $bilinguismo_images_back_nav= 'format/bilinguismo/img/back_arrow.png';
            $img_bilinguismo_nav = '<img src="' . $bilinguismo_images_back_nav . '" />';
            $backnavhtml .= $img_bilinguismo_nav;
            // $backnavhtml .= html_writer::tag('i', '', ['class' => 'fa fa-arrow-left']);
            // $backnavhtml .= $course_btn_back_lang;
            $backnavhtml .= html_writer::end_tag('a');
            $bilinguismo_images_activities = 'format/bilinguismo/img/modulo_card2.png';
            $img_bilinguismo = '<img src="' . $bilinguismo_images_activities . '" id="module-image" />';
            $backnavhtml .= $img_bilinguismo;
            $backnavhtml .= html_writer::tag('h5','Lessons',['class' => "module-title"]);
         ...
         */
           $html .= $backnavhtml;
           $html = html_writer::tag('div', $html, ['id' => 'bilinguismo-container-section']);
           if ($PAGE->user_is_editing()) {
               $load = 'M.format_bilinguismo.showmodule()';
               $html .= html_writer::tag('div', get_string('editing', 'format_bilinguismo'), ['class' => 'alert alert-warning alert-block fade in']);
    ...
    {   
        global $PAGE;
        if ($PAGE->user_is_editing()) {
            $class_editing ='bilinguismo-editing';
            return html_writer::start_tag('ul', ['class' => 'buttons row '.$class_editing. '','id'=>'rowinit']);
        }
        return html_writer::start_tag('ul', ['class' => 'buttons row bilinguismo-no-editing','id'=>'rowinit']);
    }
    /**
     * section_header
     *
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused)
    {
        ...
        //     $nameobj = (string)$course->{'moduleobjetive' . ($currentdivisor2-1)};
        // }
        // $course_Desempeno_lang = get_string('desempeno', 'format_bilinguismo');
       ...
       // echo "<p id='charge'>".get_string('cargando', 'format_bilinguismo')."</p>";
        ...
        if (!$PAGE->user_is_editing()) {
        echo html_writer::tag('span', $htmlsection0, ['class' => 'above','id'=>'above', 'style'=>'width:100%;display:none']);
        }
        else{
        ...
        if (!$PAGE->user_is_editing()) {
            $PAGE->requires->js_init_call('M.format_bilinguismo.init', [$course->numsections,$course->id]);
            echo html_writer::tag('style', '#summary { display: block !important; }');
