...
/**
 * Privacy Subsystem implementation for theme_talentum.
 *
...
namespace theme_talentum\privacy;
...
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropennavpref)) {
            $preferencestring = get_string('privacy:drawernavclosed', 'theme_talentum');
            if ($draweropennavpref == 'true') {
                $preferencestring = get_string('privacy:drawernavopen', 'theme_talentum');
            }
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_talentum');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_talentum');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_talentum',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_talentum');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_talentum');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_talentum',
